...
CREATE TABLE bookHub.Employees (
  ...
  password CHAR(64) UNIQUE NOT NULL         -- Senha do funcionário (não pode ser nula)
);
...
CREATE TABLE bookHub.Books (
  ...
  FOREIGN KEY (gender) REFERENCES bookHub.Gender (gender_id)  -- Chave estrangeira para o gênero
);
...
CREATE TABLE bookHub.BookAuthors (
  ...
  FOREIGN KEY (book_id) REFERENCES bookHub.Books (book_id),    -- Chave estrangeira para o livro
  FOREIGN KEY (author_Id) REFERENCES bookHub.Authors (author_Id)  -- Chave estrangeira para o autor
);
